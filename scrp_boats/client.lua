...
RegisterNetEvent( 'scrp:spawnBoat' )
AddEventHandler( 'scrp:spawnBoat', function ( boat )
...
RegisterCommand("dropboat", function(source, args, raw)
    if recentlySpawned <= 0 then
				recentlySpawned = 10
				TriggerServerEvent('scrp:dropboat')
			else
